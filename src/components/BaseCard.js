...
function BaseCard({ cardData }) {
	...
	return (
		<div
			key={cardData?.id}
			className="flex flex-col border rounded shadow-md bg-[#222] text-white p-2 gap-2 justify-between"
		>
			...
			{cardData?.base_attr_display !== undefined &&
				cardData?.position?.indexOf('2') === -1 &&
				cardData?.position?.indexOf('2|3') === -1 && (
					<div className="">
