...
function Skill({ skill, index, showDetail }) {
	...
	const displayAffixParam = (param, text) => {
		...
		myReturn = myReturn
			.replace('$P1$', p1Replace)
			.replace('$+P1$%', p1Replace + '%')
			.replace('$+P1$', p1Replace);
		myReturn = myReturn
			.replace('$P2$', p2Replace)
			.replace('$+P2$%', p2Replace + '%')
			.replace('$+P2$', p2Replace);
		myReturn = myReturn
			.replace('$P3$', p3Replace)
			.replace('$+P3$%', p3Replace + '%')
			.replace('$+P3$', p3Replace);
		//let myReg = /<e[^>]*>(.*?)<\/e>/img;
	...
	return (
		<div key={skill.id + '-' + index} className="flex flex-col border rounded shadow-md bg-[#222] text-white p-2">
			<div className="flex flex-row gap-2 items-start">
				<div>
					<img loading="lazy" src={`img/icons/CoreTalentIcon/${skill.icon}.png`} className="w-[64px]" alt="Icon" />
				</div>
								...
								{Array.isArray(affix.param) &&
									affix.param.map(
										(p) =>
											Array.isArray(p) &&
											p
												.sort((a, b) => parseInt(a.level) - parseInt(skill.level))
												.map((pdata) => (
													<div key={pdata.level + '-' + pdata.value}>
														(Level {pdata.level} : {pdata.value})
													</div>
												))
									)}
							</div>
