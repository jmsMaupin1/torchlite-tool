							...
							<div key={i} className="prefix">
								{s['tier0_' + i18n.language][0] !== null && currentDisplay === 0 && (
									<div className="flex flex-row gap-2 items-center ">
										...
										<div className="flex flex-col">
											{s['tier0_' + i18n.language].map((affix, ind) => (
								...
								{currentDisplay === 1 && (
									<div className="flex flex-row gap-2 items-center">
										...
										<div className="flex flex-col">
											{s['tier1_' + i18n.language].length > 1 ? <div></div> : null}
											{s['tier1_' + i18n.language].map((affix, ind) => (
												<React.Fragment key={ind}>
													{ind > 0 && <div className="text-center text-[10px]">╼ {t('commons:OR')} ╾</div>}
													<HyperLinkTooltip key={'prefix-' + i} str={hightlightSearch(affix)} />
												</React.Fragment>
											))}
										</div>
									</div>
								...
								{s['tier0_' + i18n.language][0] !== null && currentDisplay === 0 && (
									<div className="flex flex-row gap-2 items-start">
										...
										<HyperLinkTooltip
											...
											className="font-bold bg-clip-text text-transparent bg-gradient-to-b from-purple-400 to-purple-900"
											key={'prefix-' + i}
											str={hightlightSearch(s['tier0_' + i18n.language])}
										/>
									</div>
								)}
								{currentDisplay === 1 && (
									<div className="flex flex-row gap-2 items-start">
										...
										<HyperLinkTooltip str={hightlightSearch(s['tier1_' + i18n.language])} />
									</div>
								)}
							</div>
						))}
