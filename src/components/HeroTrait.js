...
function HeroTrait(props) {
    ...
    return (
            ...
            {[1].map((index) => (
                <div key={index} className='flex flex-col items-center w-full'>
                    <div className='flex flex-row items-center gap-2 w-full'>
                        <div><img src={`img/hero/${h.portrait2}.png`} className='h-20 w-18 min-w-18' alt="Hero"/></div>
                        <div className='title text-sm'>{translate(h.name).split('|')[1]}</div>
                    </div>
                    {perk.filter((p) => p.hero_id === h.id && p.level === index.toString()).map((p) => (
                        <div key={p.character_id} className='w-full flex shadow-md shadow-black p-1 border-t border-black items-center justify-center flex-col'>
        ...
        <div key={h.id} className='bg-[#282828] flex flex-col gap-1 md:flex-row justify-between w-full border p-1 rounded-lg shadow-lg  flex-wrap'>
