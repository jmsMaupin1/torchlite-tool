	...
	return (
		<>
			<div className={`absolute z-10 w-50 flex flex-row ${!visible ? 'hidden' : ''}`}>
				{returnSlice().map((tree, index) => (
					...
					>
						<div
							onClick={() => {
								onSpecCoreChange(tree.id, coreNumber, tree.attr);
								setVisible(false);
							}}
							className={`bg-slate-900 border rounded-md p-2 hover:cursor-pointer items-center flex flex-col ${
								index === 2 ? 'mr-10' : ''
							} `}
						>
							<div>
			...
			{returnCoreNumber() != null ? (
				currentTree
					.filter((e) => e.position === '0|0' && e.id === returnCoreNumber())
					.map((tree, index) => (
						<Tooltip
							key={tree.id + '-' + index}
							content={
						...
						>
							<div
								...
								className={`relative border rounded-md p-2 hover:cursor-pointer items-center flex flex-col ${
									index === 2 ? 'mr-10' : ''
								} `}
							>
								<div>
				...
				<div
					onClick={() => setVisible(!visible)}
					style={{ fontSize: '80px' }}
					className="grow-0 hover:cursor-pointer border rounded-full"
				>
					+
				</div>
			)}
