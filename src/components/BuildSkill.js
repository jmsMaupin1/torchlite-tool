...
function BuildSkill(props) {
    ...
    return (
        ...
        <div className='flex flex-row relative'>
            ...
            <div onClick={() => setIsVisible(!isVisible)} style={{fontSize:"80px"}} className='w-20 h-20 hover:cursor-pointer border rounded-full items-center justify-center flex flex-col'>
                {skill !== null ? 
                    <div>
                        <img loading="lazy" src={`img/icons/skills/${skill.icon}.png`} className="w-[80px]" alt="Icon"/>
                    </div>
                    :
                    <div className='-mt-[14px]'>+</div>
                }
            </div>
            {skill !== null ? <div className='text-sm'><span>{translate(skill.name)}</span></div>:null}
            </div>
            <div className='w-full'>
            <div className='grid grid-cols-5 w-full gap-2'>
                {[1,2,3,4,5].map((index) => (
                    <div key={index} className='w-full flex flex-col justify-center items-center'>
                        <div onClick={() => _setIsSupportVisible(index)} style={{fontSize:"80px"}} className='w-20 h-20 hover:cursor-pointer border rounded-full items-center justify-center flex flex-col'>
                            {tabSupport[index] !== null ? 
                                <>
                                <div>
                                    <img loading="lazy" src={`img/icons/skills/${tabSupport[index].icon}.png`} className="w-[80px]" alt="Icon"/>
                                </div>
                                </>
                                :
                                <div className='-mt-[14px]'>+</div>
